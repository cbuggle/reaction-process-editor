...
const RemoveForm = ({ workup, preconditions, onWorkupChange }) => {
  ...
  const handleAmountChange = (name) => (amount) =>
    onWorkupChange({ name: name, value: amount });
  ...
  const renderConditions = () => {
    return removeFormMetricNames.map((metricName) => (
      <>
        <MetricsInput
    ...
    return (
      <>
        ...
        <MetricsInput
          ...
          amount={
            workup["remove_repetitions"] ||
            MetricsDecorator.defaultAmount("REPETITIONS")
          }
  ...
  const renderGenericRemoveFields = () => {
    switch (workup["acts_as"]) {
      case "ADDITIVE":
        return renderSolventRemoveFields(
          "Solvent (Additive)",
          additivesSelectOptions
        );
      case "DIVERSE_SOLVENT":
        return renderSolventRemoveFields(
          "Solvent (Diverse)",
          diverseSolventsSelectOptions
        );
      case "MEDIUM":
