// This is a clone of react-numeric-input slightly adapted and
// slightly renamed to distinguish, as required for the KIT ELN.
// The original code has last changed in Jan 2018 so we don't expect any updates anytime soon.
//
// It fixes an issue with the stepper not being able to reach the max value if it isn't exactly on a step.
// e.g. 0.1 -> 0.2 -> 0.25 was impossible as the snapper would enforce 0.1 -> 0.2 -> 0.2
// See _step()
//
// Also lots of !=, == warnings have been fixed.
// cbuggle, 03.10.2023
...
class NumericalInput extends Component {
  ...
  _toNumber(x: any): number {
    ...
    if (this._isStrict) {
      ...
      let _min = +access(this.props, "min", NumericalInput.defaultProps.min, this);
  ...
  _step(n: number, callback?: Function): boolean {
    ...
    let _step = +access(
      this.props,
      "step",
      NumericalInput.defaultProps.step,
      this,
      (
        n > 0 ?
          NumericalInput.DIRECTION_UP :
          NumericalInput.DIRECTION_DOWN
      )
    );
  ...
  increase(_recursive: boolean = false, callback?: Function): void {
    ...
    if (isNaN(this.state.value) || +this.state.value < _max) {
      this._timer = setTimeout(() => {
        this.increase(true);
      }, _recursive ? NumericalInput.SPEED : NumericalInput.DELAY);
    }
  }
  ...
  render() {
    ...
    // Build the styles
    for (let x in NumericalInput.style) {
      css[x] = Object.assign(
        {},
        NumericalInput.style[x],
        style ? style[x] || {} : {}
      );
    }
